...
class GaussianMixtureMLP(MLP):
    ...
    def forward(self, x, mean, rho, coef, logvar_enc=False, mean_only=False,
        ...
        assert len(mean) == len(self.param_shapes) and \
               len(rho) == len(self.param_shapes) and \
                len(coef) == len(self.param_shapes)

        for i, s in enumerate(self.param_shapes):
            assert np.all(np.equal(s, list(mean[i].shape))) and \
                   np.all(np.equal(s, list(rho[i].shape))) and \
                    np.all(np.equal(s, list(coef[i].shape)))
        ...
        for i, p in enumerate(mean):
            if self.has_bias and i % 2 == 1:
                b_means.append(p)
                b_vars.append(var[i])
                b_coefs.append(coef[i])
            else:
                w_means.append(p)
                w_vars.append(var[i])
                w_coefs.append(coef[i])
