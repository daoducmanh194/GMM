...
from probabilistic import GaussianMixtureBNNWrapper
from probabilistic import train_vi_mixture
from probabilistic.prob_cifar import train_utils_mixture as pcutils
from probabilistic.prob_gmm import train_utils_mixture as pgutils
from probabilistic.prob_mnist import train_utils_mixture
from probabilistic.regression import train_utils_mixture as rutils
...
def evaluate(task_id, data, mnet, hnet, device, config, shared, logger, writer,
    ...
    with torch.no_grad():
        ...
        acc, ret_vals = train_utils_mixture.compute_acc(task_id, data, mnet, hnet,
...
def distill_net(task_id, data, mnet, hnet, hhnet, device, config, shared,
    ...
    assert not isinstance(mnet, GaussianMixtureBNNWrapper) or hhnet is None
    ...
    if distill_mnet:
        mnet.eval()
        with torch.no_grad():
            mnet_acc, _ = train_utils_mixture.compute_acc(task_id, data, mnet, None,
    ...
    if distill_mnet:
        ...
        if isinstance(mnet, GaussianMixtureBNNWrapper) and \
                config.regularizer != 'mse':
            mnet_w_mean, mnet_w_rho, mnet_coef = mnet.extract_mean_rho_coef(weights=None)
            for gauss in range(config.gauss_mixture):
                _, mnet_w_logvar_tmp = putils.decode_diag_gauss(mnet_w_rho[gauss], \
                                                        logvar_enc=mnet.logvar_encoding[gauss],
                                                        return_logvar=True)
                mnet_w_logvar.append(mnet_w_logvar_tmp)

            mnet_w_mean = [p.detach().clone() for p in mnet_w_mean]
            mnet_w_logvar = [p.detach().clone() for p in mnet_w_logvar]
            mnet_w_coef = [p.detach().clone() for p in mnet_w_coef]
    ...
    if distill_mnet:
        regged_outputs = train_utils_mixture.calc_regged_out_inds(config, task_id, data)
    else:
    ...
    if calc_reg:
        ...
        if distill_mnet:
            if config.calc_hnet_reg_targets_online:
                ...
                targets, target_means, target_logvars, target_coefs = \
                    train_utils_mixture.calc_reg_target(config, task_id, hnet,
                                                mnet=mnet)
        else:
            if config.calc_hnet_reg_targets_online:
    ...
    for i in range(config.distill_iter):
        ...
        if i % config.val_iter == 0:
            ...
            with torch.no_grad():
                ...
                if distill_mnet:
                    hnet_acc, _ = train_utils_mixture.compute_acc(task_id, data, mnet,
        ...
        if distill_mnet:
            ...
            if mnet_w_mean is not None:
                ...
                w_logvar = []
                for gauss in range(config.gauss_mixture):
                    w_std_tmp, w_logvar_tmp = putils.decode_diag_gauss(w_rho[gauss], \
                                                           logvar_enc=mnet.logvar_encoding[gauss],
                                                            return_logvar=True)
                    w_std.append(w_std_tmp)
                    w_logvar.append(w_logvar_tmp)
        else:
            hnet_out = hhnet.forward(cond_id=task_id)

        ### Compute distillation loss.
        # FIXME We could apply masking in the multi-head case.
        if not isinstance(mnet, GaussianMixtureBNNWrapper) or \
                config.regularizer == 'mse':
            distilled_w = torch.cat([w.flatten() for w in hnet_out])
            dis_loss = (new_target - distilled_w).pow(2).sum()
        else:
            dis_loss = train_utils_mixture.calc_gauss_reg(config, task_id, mnet, hnet,
                                                  target_mean=mnet_w_mean, target_logvar=mnet_w_logvar, target_coef=mnet_w_coef,
                                                  current_mean=w_mean, current_logvar=w_logvar, current_coef=w_coef)
...
def train(task_id, data, mnet, hnet, device, config, shared, logger, writer):
    ...
    # task.
    allowed_outputs = train_utils_mixture.out_units_of_task(config, data, task_id,
                                                    task_id + 1)
    ...
    if config.use_prev_post_as_prior and task_id > 0:
        ...
        w_mean_prev, w_rho_prev, w_coef_prev = mnet.extract_mean_rho_coef(weights=hnet_out)
        w_std_prev = []
        w_logvar_prev = []
        for gauss in range(config.gauss_mixture):
            w_std_prev_tmp, w_logvar_prev_tmp = putils.decode_diag_gauss(w_rho_prev[gauss], \
                                                            logvar_enc=mnet.logvar_encoding[gauss],
                                                            return_logvar=True)
            w_std_prev.append(w_std_prev_tmp)
            w_logvar_prev.append(w_logvar_prev_tmp)
        ...
        prior_coef = [p.detach().clone() for p in w_coef_prev]
    else:
    ...
    if calc_reg:
        if config.calc_hnet_reg_targets_online:
            ...
            targets, target_means, target_logvars, target_coefs = \
                train_utils_mixture.calc_reg_target(config, task_id, hnet, mnet=mnet)
            prev_hnet_theta = None
            prev_task_embs = None
    ...
    ### Start training ###
    ######################
    mnet_kwargs = train_utils_mixture.mnet_kwargs(config, task_id, mnet)
    ...
    for i in range(num_train_iter):
        ...
        if perform_pm:
            if not config.radial_bnn:
                if standard_prior:
                    ...
                    loss_kl = putils.upperbound_kl_mixture_gauss([w_mean, w_logvar, w_coef],
                                                                [prior_mean, prior_logvar, prior_coef])
            else:
                ...
                loss_kl = putils.kl_radial_bnn_with_diag_gauss(w_mean, w_std,
                                                               ...
                                                               ce_sample_size=config.num_kl_samples)
        else:
            loss_kl = 0
        ...
        if config.coreset_size != -1 and \
            ...
            for prev_task_id in range(task_id):
                ...
                with torch.no_grad():
                    mnet_kwargs_prev = train_utils_mixture.mnet_kwargs(config, \
                                                               prev_task_id, mnet)
                ...
                for j in range(config.train_sample_size):
                    ...
                    if allowed_outputs is not None:
                        ...
                        allowed_outputs_prev = train_utils_mixture.out_units_of_task( \
                            config, data,
                            prev_task_id,
                            task_id)
        ...
        kl_scale = train_utils_mixture.calc_kl_scale(config, num_train_iter, i, logger)
        ...
        ###############################
        train_utils_mixture.apply_lr_schedulers(config, shared, logger, task_id, data,
    ...
    train_utils_mixture.checkpoint_bn_stats(config, task_id, mnet)
    ...
    if config.coreset_size != -1 and not (config.past_and_future_coresets or \
        ...
        train_utils_mixture.update_coreset(config, shared, task_id, data, mnet, hnet,
...
def train_multitask_coresets(dhandlers, mnet, hnet, device, config, shared,
    ...
    for task_id in range(config.num_tasks):
        ...
        prior_mean, prior_rho = mnet.extract_mean_rho_coef(weights=hnet_out)
        for gauss in range(config.gauss_mixture):
            prior_std[gauss], prior_logvar[gauss] = putils.decode_diag_gauss(prior_rho[gauss],
                                                        logvar_enc=mnet.logvar_encoding[gauss],
                                                        return_logvar=True)
    ...
    for i in range(num_train_iter):
        ...
        for task_id, data in enumerate(dhandlers):
            ...
            # current task.
            allowed_outputs = train_utils_mixture.out_units_of_task(config, data,
                                                            task_id, task_id + 1)
            ...
            if i > 0:  # Note, the KL is zero in the first iteration.
                ...
                if not config.radial_bnn:
                    ...
                    loss_kl_curr_task = putils.upperbound_kl_mixture_gauss([w_mean, w_logvar, w_coef],
                                            [priors_mean[task_id], priors_logvar[task_id], priors_coef[task_id]])
            ...
            cs_trgts = train_utils_mixture.fit_targets_to_softmax(config, shared,
...
def run(config, experiment='split_bbb'):
    ...
    ### Create tasks.
    dhandlers = train_utils_mixture.load_datasets(config, logger, experiment, writer)
    ...
    mnet, hnet = train_utils_mixture.generate_gauss_networks(config, logger,
                                                            dhandlers, device,
                                                            experiment,
                                                            no_mnet_weights=no_mnet_weights,
    ...
    # Mean, variance, coef of prior that is used for variational inference.
    if config.mean_only:  # No prior-matching can be performed.
        ...
        # Mean, variance, coef type of list [num_gauss, ...]
        for gauss in range(config.gauss_mixture):
            shared.prior_meam.append([torch.zeros(*s).to(device) \
                                for s in mnet.orig_param_shapes])
            shared.prior_logvar.append([plogvar * torch.ones(*s).to(device) \
                                for s in mnet.orig_param_shapes])
            shared.prior_std.append([pstd * torch.ones(*s).to(device) \
                                for s in mnet.orig_param_shapes])
            shared.prior_coef.append([torch.ones(*s).to(device) \
                                for s in mnet.orig_param_shapes])
    ...
    # ones). Note, weights refer to sum of coef * (mean ,variances)
    #  (e.g., the output of the hypernetwork).
    ...
    if config.coreset_size != -1 and (config.past_and_future_coresets or \
        ...
        for i in range(config.num_tasks):
            train_utils_mixture.update_coreset(config, shared, i, dhandlers[i], None,
    ...
    if config.coreset_size != -1 and config.final_coresets_finetune:
        train_utils_mixture.remove_coreset_from_training_data(config, logger,
                                                      dhandlers, shared)
    ...
    during_acc_criterion = train_utils_mixture.parse_performance_criterion(config,
                                                                   shared, logger)
    ...
    for i in range(config.num_tasks):
        ...
        if config.distill_iter == -1:
            ...
            mnet, _ = train_utils_mixture.generate_gauss_networks(config, logger,
        ...
        train_vi_mixture.test(dhandlers[:(i + 1)], mnet, hnet, None, device, config,
        ...
        if config.train_from_scratch and i < config.num_tasks - 1:
            ...
            train_utils_mixture.checkpoint_nets(config, shared, i, mnet, hnet)
    ...
    if config.final_coresets_finetune:
        ...
        train_vi_mixture.test(dhandlers, mnet, hnet, None, device, config, shared,
                            logger, writer, method='bbb')

    if config.store_final_model:
        logger.info('Checkpointing final model ...')
        train_utils_mixture.checkpoint_nets(config, shared, config.num_tasks - 1, mnet,
                                            hnet)
    ...
    train_utils_mixture.save_summary_dict(config, shared, experiment)
