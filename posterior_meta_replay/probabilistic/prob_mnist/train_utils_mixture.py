...
def compute_acc(task_id, data, mnet, hnet, device, config, shared,
    ...
    gauss_main = isinstance(mnet, GaussianMixtureBNNWrapper)
    ...
    if normal_post is not None:
        ...
        for gauss in range(config.gauss_mixture):
            w_std_tmp = putils.decode_diag_gauss(w_rho[gauss], logvar_enc=mnet.logvar_encoding[gauss])
    ...
    for j in range(num_w_samples):
        ...
        if w_samples is not None:
            ...
            W = putils.sample_from_gumbel_softmax_trick(w_mean, w_std, w_coef,
                                                        tau=1.0, gauss_mixture=3,
                                                        K=1, generator=generator,
                                                        is_radial=config.radial_bnn)
            emean = None
            erho = None
            ecoef = None
...
def calc_reg_target(config, task_id, hnet, mnet=None):
    ...
    if hasattr(config, 'regularizer') and config.regularizer != 'mse':
        ...
        for i in range(task_id):
            ...
            for gauss in range(config.gauss_mixture):
                _, target_logvars[gauss][i] = putils.decode_diag_gauss(rho[gauss], \
...
def calc_gauss_reg_all_tasks(config, task_id, mnet, hnet, target_means=None,
                             target_logvars=None, target_coefs=None,
...
def calc_gauss_reg(config, task_id, mnet, hnet, target_mean=None,
                   target_logvar=None, target_coef = None, 
                   current_mean=None, current_logvar=None,
                   current_coef = None, prev_theta=None, prev_task_embs=None):
    ...
    if prev_theta is not None:
        ...
        with torch.no_grad():
            ...
            target_mean, rho, coef = mnet.extract_mean_rho_coef(weights=hnet_out)
            for gauss in range(config.gauss_mixture):
                _, target_logvar[gauss] = putils.decode_diag_gauss(rho[gauss], \
                                                        logvar_enc=mnet.logvar_encoding[gauss],
                                                        return_logvar=True)

        target_mean = [p.detach().clone() for p in target_mean]
        target_logvar = [p.detach().clone() for p in target_logvar]
        target_coef = [p.detach().clone() for p in target_coef]
    ...
    if current_mean is None:
        ...
        w_mean, w_rho, w_coef = mnet.extract_mean_rho_coef( \
            weights=hnet_out)
        for gauss in range(config.gauss):
            _, w_logvar[gauss] = putils.decode_diag_gauss(w_rho[gauss], \
                                                logvar_enc=mnet.logvar_encoding[gauss],
                                                return_logvar=True)
    else:
        w_mean = current_mean
