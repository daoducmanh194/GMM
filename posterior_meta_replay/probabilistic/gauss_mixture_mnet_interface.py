"""
A main network wrapper that handles Gaussian Mixture network weights
------------------------------------------------------------

The module :mod:`probabilistic.gauss_mixture_mnet_interface` contains a wrapper
for class :class:`mnets.mnet_interface.MainNetInterface` is implemented, that
triplicates internal weights to represent mean :math:`\mu` and variance
:math:`\sigma^2` and coeficient :math:'\coef'. Additionally, the
...
class GaussianMixtureBNNWrapper(nn.Module, MainNetInterface):
    r"""Convert Main Network into BNN with Gaussian Mixture weight distribution.
    ...
    described below) of a Gaussian distribution and coeficient :math:'\coef'.
    Note, the weight distribution is fully-factorized,
    i.e., correlations between weights cannot be learned.
    ...
    Args:
        ...
        is_radial (bool): Indicates whether the instantiated network should be a
            Radial BNN.
        gauss_mixture (int): Number of Gaussian components to approximate
    ...
    def __init__(self, mnet, no_mean_reinit=False, logvar_encoding=False,
                apply_rho_offset=False, is_radial=False, gauss_mixture=3):
        ...
        self._gauss_mixture = gauss_mixture

        # Take over attributes of 'mnet' and modify them if necessary
        self._mean_params = None
        self._rho_params = None
        self._coef_params = None
        if mnet.internal_params is not None:
            self._mean_params = mnet.internal_params
            self._rho_params = nn.ParameterList()
            self._coef_params = nn.ParameterList()

            for p in self._mean_params:
                self._rho_params.append(nn.Parameter(torch.Tensor(p.sizse()),
                                                    requires_grad=True))
                self._coef_params.append(nn.Parameter(torch.Tensor(p.zise()), 
                                                    requires_grad=True))

            # Initialize weights
            if not no_mean_reinit:
                for p in self._mean_params:
                    p.data.uniform(-0.1, 0.1)

            for p in self._rho_params:
                if apply_rho_offset:
                    # subtract 2.5 from 'rho' in the forward mehthod
                    # p.data.uniform(-0.5, 0.5)
                    p.data.uniform(-3-self._rho_offset, -2-self._rho_offset)
                else:
                    p.data.uniform(-3, -2)
        ...
        if mnet._hyper_shapes_learned_ref is not None:
            ...
            for iii in mnet._hyper_shapes_learned_ref:
                self._hyper_shapes_learned_ref.append(iii + 2*old_plen)
    ...
    def layer_weight_tensors(self):
        ...
        """
        warn('Class "GaussianMixtureBNNWrapper" didn\'t modify the attribute ' +
    ...
    @property
    def coef(self):
        """ Gettter for read-only attribute :attr:'coef'.

        Returns:
            (torch.nn.ParameterList): List of internally stored coefficients
            parameters of ''None'' if no parameters are maintained internally.
        """
        return self._coef_params
    ...
    def distillation_targets(self):
        ...
        # is doing.
        warn('Class "GaussianMixtureBNNWrapper" doesn\'t modify the method ' +
    ...
    def forward(self, x, weights=None, distilled_params=None, condition=None,
                ...
                extracted_rho=None, extracted_coef=None, sample=None,
                disable_lrt=False, rand_state=None, **kwargs):
        ...
        Args:
            ...
            extracted_mean (optional): The first return value of method
                ...
                :meth:`extract_mean_rho_coef`. Passed ``weights`` will be ignored
                if this argument is specified.

                Requires that ``extracted_rho`` is set as well.
            extracted_rho (optional): The second return value of method
                :meth:`extract_mean_rho_coef`. See description of
                ``extracted_mean``.
            extracted_coef (optional): The third return value of method
                :meth:'extracted_mean_rho_coef'
        ...
        assert (extracted_mean is None and extracted_rho is None 
        ...
        if isinstance(self._mnet, GaussianMLP) and not disable_lrt:
            ...
            if ret_sample:
                raise ValueError('Arguement "ret_sample" is not applicable, ' +
                                'when using local reparametrization trick.')

            y = self._mnet.forward(x, mean, rho, coef, 
    ...
    def extract_mean_rho_coef(self, weights=None):
        """Extract all means and rho-values and coef from internally maintained
        and/or externally provided weights.
...
mnet_interface.MainNetInterface.hyper_shapes_learned`
            ...
            mean_only (bool, optional): Rather than drawing random sample, the
                mean (:attr:`mean`) would be used.
            extracted_mean (optional): The first return value of method
                :meth:`extract_mean_rho_coef`. Can be passed for compuational
                efficiency reasons (if mean- and rho-values where already
                computed). Otherwise, this method will simply call
        ...
        if extracted_mean:
            ...
            assert len(extracted_mean) == len(extracted_coef) and \
                len(extracted_mean) == len(extracted_rho) and \
                len(extracted_mean) == len(self._mnet.param_shapes)
