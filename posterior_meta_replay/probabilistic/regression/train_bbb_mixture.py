...
from probabilistic.prob_mnist import train_utils_mixture as pmutils
from probabilistic.regression import train_args
from probabilistic.regression import train_utils_mixture
...
def test(data_handlers, mnet, hnet, device, config, shared, logger, writer,
    ...
    with torch.no_grad():
        ...
        if config.train_from_scratch and n > 1:
            ...
            if 'avb' in shared.experiment_type \
                ...
                mnet_other, hnet_other = train_utils_mixture.generate_gauss_networks( \
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                mse_val, val_struct = train_utils_mixture.compute_mse(j, data, mnet_j,
    ...
    train_utils_mixture.plot_mse(config, writer, n, shared.current_mse[:n],
    ...
    train_utils_mixture.plot_predictive_distributions(config, writer, data_handlers,
    ...
    train_utils_mixture.save_summary_dict(config, shared)
...
def evaluate(task_id, data, mnet, hnet, device, config, shared, logger, writer,
    ...
    with torch.no_grad():
        ...
        mse_val, val_struct = train_utils_mixture.compute_mse(task_id, data, mnet,
        ...
        mse_test, test_struct = train_utils_mixture.compute_mse(task_id, data, mnet,
        ...
        if config.show_plots or train_iter is not None:
            train_utils_mixture.plot_predictive_distribution(data, test_struct.inputs,
...
def train(task_id, data, mnet, hnet, device, config, shared, logger, writer):
    ...
    """
    assert isinstance(mnet, GaussianMixtureBNNWrapper) or config.mean_only
    ...
    if calc_reg:
        targets, w_mean_pre, w_logvar_pre, w_coef_pre = pmutils.calc_reg_target(config,
    ...
    if config.use_prev_post_as_prior and task_id > 0:
        ...
        for gauss in range(config.gauss_mixture):
            ...
            w_logvar_prev.apeend(w_logvar_prev_tmp)
        ...
        if calc_reg:
            if config.regularizer == 'mse':
                ...
                for t in range(task_id):
                    ...
                    w_logvar_t = []
                    for gauss in range(config.gauss_mixture):
                        _, w_logvar_t_tmp = putils.decode_diag_gauss(w_rho_t[gauss], \
                                                            logvar_enc=mnet.logvar_encoding[gauss],
                                                            return_logvar=True)
                        w_logvar_t.append(w_logvar_t_tmp)
...
def run():
    ...
    train_utils_mixture.backup_cli_command(config)

    ### Create tasks.
    dhandlers, num_tasks = train_utils_mixture.generate_tasks(config, writer)

    ### Generate networks.
    use_hnet = not config.mnet_only
    mnet, hnet = train_utils_mixture.generate_gauss_networks(config, logger, dhandlers,
    ...
    ### training.
    train_utils_mixture.setup_summary_dict(config, shared, 'bbb', num_tasks, mnet,
                                   hnet=hnet)
