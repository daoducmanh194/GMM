...
def train(task_id, data, mnet, hnet, hhnet, dis, device, config, shared, logger,
    ...
    if perform_pm:
        prior_mean, prior_std, prior_coef, prior_dist, prior_theta = None, None, None, None, None
        ...
        if use_explicit_prior:
            ...
            prior_coef = torch.cat([p.flatten() for p in shared.prior_coef])
        ...
        if perform_pm:
            if method == 'avb':
                ...
                loss_kl, w_samples = pcutils.calc_prior_matching(config, shared,
                                                                 config.num_kl_samples, device, dis, hnet,
                                                                 theta_current,
                                                                 prior_dist, ac_dist,
